...
namespace Crosstales.NLayer
{
   public class MpegFile : IDisposable
   {
      ...
      {
         get { return _reader.SampleRate; }
      }

      public int Channels
      {
         get { return _reader.Channels; }
      }
      ...
      public long Length
      {
         get { return _reader.SampleCount * _reader.Channels * sizeof(float); }
      }
         ...
         lock (_seekLock)
         {
            while (count > 0)
            {
               if (_readBufLen > _readBufOfs)
               {
                  ...
                  //Array.Copy(_readBuf, _readBufOfs, buffer, index, temp); //#CT: changed
               ...
               if (_readBufLen == 0)
               {
                  ...
                  {
                     //Debug.LogError("InvalidDataException: " + ex.StackTrace);
                  ...
                  {
                     //Debug.LogError("EndOfStreamException: " + ex.StackTrace);
