...
public class Music_Player : MonoBehaviour
{
    ...
    private Camera camera_visualizator_component;
    private AudioListener listener_visualizator;
    ...
    private bool is_rotate_audio_wave = false;
    public float speed_rotate_audio_wave = 18f;
    public bool is_cinematic_wave_camera = true;
    public float cinematic_move_interval = 4f;
    public float cinematic_lerp_speed = 1.75f;
    public Vector2 cinematic_yaw_range = new Vector2(-165f, 165f);
    public Vector2 cinematic_pitch_range = new Vector2(10f, 38f);
    public Vector2 cinematic_distance_range = new Vector2(26f, 50f);
    private MusicWaveSkybox waveSkybox;
    private bool cached_rhythm_camera_control = true;
    private bool has_cinematic_shot = false;
    private float cinematic_timer = 0f;
    private Vector3 cinematic_shot_from_pos;
    private Vector3 cinematic_shot_to_pos;
    private Quaternion cinematic_shot_from_rot;
    private Quaternion cinematic_shot_to_rot;
    ...
    void Start()
    {
        ...
        this.waveSkybox = FindObjectOfType<MusicWaveSkybox>();
        if (this.waveSkybox == null)
        {
            GameObject objWaveSkybox = new GameObject("MusicWaveSkybox");
            this.waveSkybox = objWaveSkybox.AddComponent<MusicWaveSkybox>();
            this.waveSkybox.app = this.app;
            this.waveSkybox.musicSource = this.GetComponent<AudioSource>();
            this.waveSkybox.targetCamera = this.camera_visualizator_component;
        }

        this.Set_wave_camera_state(false);
    }
    ...
    public void show_audio_wave()
    {
        ...
        this.Set_wave_camera_state(true);
        this.rhythmVisualizator.gameObject.SetActive(true);
        this.is_show_audio_wave = true;
        if (this.rhythmVisualizator != null)
        {
            this.cached_rhythm_camera_control = this.rhythmVisualizator.cameraControl;
            this.rhythmVisualizator.cameraControl = false;
        }
        if (this.waveSkybox != null)
        {
            this.waveSkybox.targetCamera = this.camera_visualizator_component;
            this.waveSkybox.SetWaveMode(true);
        }
        this.Init_cinematic_wave_camera();
    }
    ...
    public void hide_audio_wave()
    {
        ...
        this.has_cinematic_shot = false;
        if (this.rhythmVisualizator != null) this.rhythmVisualizator.cameraControl = this.cached_rhythm_camera_control;
        if (this.waveSkybox != null) this.waveSkybox.SetWaveMode(false);
    }

    private void Setup_wave_cameras()
    {
        if (this.camera_main == null)
        {
            Camera camMain = Camera.main;
            if (camMain != null) this.camera_main = camMain.gameObject;
        }

        if (this.camrea_visualizator == null)
        {
            GameObject camWaveObj = GameObject.Find("ExampleCamera");
            if (camWaveObj != null) this.camrea_visualizator = camWaveObj;
        }

        if (this.camrea_visualizator != null) this.camera_visualizator_component = this.camrea_visualizator.GetComponent<Camera>();
        if (this.camrea_visualizator != null)
        {
            this.listener_visualizator = this.camrea_visualizator.GetComponent<AudioListener>();
            if (this.listener_visualizator != null) this.listener_visualizator.enabled = false;
        }
    }

    private void Set_wave_camera_state(bool is_wave)
    {
        if (this.camrea_visualizator != null) this.camrea_visualizator.SetActive(is_wave);
        if (this.listener_visualizator != null) this.listener_visualizator.enabled = false;
    }

    private void Init_cinematic_wave_camera()
    {
        this.has_cinematic_shot = false;
        this.cinematic_timer = 0f;
        this.Try_pick_next_cinematic_shot(true);
    }

    private void Update_cinematic_wave_camera()
    {
        if (!this.is_cinematic_wave_camera) return;
        if (this.camera_visualizator_component == null) return;
        if (this.rhythmVisualizator == null) return;

        if (!this.has_cinematic_shot)
        {
            this.Try_pick_next_cinematic_shot(true);
            if (!this.has_cinematic_shot) return;
        }

        this.cinematic_timer += Time.deltaTime;
        float duration = Mathf.Max(0.5f, this.cinematic_move_interval);
        float t = Mathf.Clamp01(this.cinematic_timer / duration);
        float smooth_t = Mathf.SmoothStep(0f, 1f, t * Mathf.Clamp(this.cinematic_lerp_speed, 0.2f, 3f));

        this.camera_visualizator_component.transform.position = Vector3.Lerp(this.cinematic_shot_from_pos, this.cinematic_shot_to_pos, smooth_t);
        this.camera_visualizator_component.transform.rotation = Quaternion.Slerp(this.cinematic_shot_from_rot, this.cinematic_shot_to_rot, smooth_t);

        if (this.cinematic_timer >= duration)
        {
            this.Try_pick_next_cinematic_shot(false);
        }
    }

    private void Try_pick_next_cinematic_shot(bool is_first_shot)
    {
        if (this.camera_visualizator_component == null || this.rhythmVisualizator == null) return;

        Transform cam_tf = this.camera_visualizator_component.transform;
        Vector3 focus = this.rhythmVisualizator.transform.position + Vector3.up * 4f;

        float yaw = Random.Range(this.cinematic_yaw_range.x, this.cinematic_yaw_range.y);
        float pitch = Random.Range(this.cinematic_pitch_range.x, this.cinematic_pitch_range.y);
        float distance = Random.Range(this.cinematic_distance_range.x, this.cinematic_distance_range.y);

        Vector3 dir = Quaternion.Euler(pitch, yaw, 0f) * Vector3.forward;
        Vector3 target_pos = focus - dir * distance;
        Quaternion target_rot = Quaternion.LookRotation(focus - target_pos, Vector3.up);

        this.cinematic_shot_from_pos = cam_tf.position;
        this.cinematic_shot_from_rot = cam_tf.rotation;
        if (is_first_shot)
        {
            this.cinematic_shot_from_pos = target_pos;
            this.cinematic_shot_from_rot = target_rot;
            cam_tf.position = target_pos;
            cam_tf.rotation = target_rot;
        }

        this.cinematic_shot_to_pos = target_pos;
        this.cinematic_shot_to_rot = target_rot;
        this.cinematic_timer = 0f;
        this.has_cinematic_shot = true;
    }
    ...
    public void change_rotate_audio_wave()
    {
        ...
        this.is_rotate_audio_wave = !this.is_rotate_audio_wave;
        // Keep camera static; rotate the visualizer block itself for full-screen wave mode.
        this.rhythmVisualizator.rotateCamera = false;

        if (this.is_rotate_audio_wave)
        {
            if (this.icon_wave_rotate != null && this.icon_wave_rotate.Length > 0)
                this.img_aduo_wave_rotate.sprite = this.icon_wave_rotate[0];
        }
        else
        {
            if (this.icon_wave_rotate != null && this.icon_wave_rotate.Length > 1)
    ...
    public void Play_by_data(IDictionary data)
    {
        ...
        )
        {
            ...
            {
                if (data["mp3"].ToString() != "") this.download_music(app.carrot.hub.GetUrlFile(data["mp3"].ToString()));
            }
            else
            {
                ...
                {
                    string url_mp3 = "file://" + app.carrot.get_tool().get_file_path(path_file);
                    Debug.Log(url_mp3);
    ...
    void Update()
    {
        if (this.is_show_audio_wave)
        {
            this.Update_cinematic_wave_camera();
        }

        if (this.is_show_audio_wave && this.is_rotate_audio_wave && this.rhythmVisualizator != null)
        {
            this.rhythmVisualizator.transform.Rotate(0f, this.speed_rotate_audio_wave * Time.deltaTime, 0f, Space.World);
        }
    ...
    {
        if (this.data_music_cur == null || this.data_music_cur["type"] == null) return;

        string type = this.data_music_cur["type"].ToString();
        bool isSaved = false;

        if (type == "music_online")
        {
            if (this.data_music_save == null || this.data_music_save.Length == 0)
            {
                app.carrot.Show_msg(app.carrot.L("playlist", "Playlist"), app.carrot.L("save_song_loading", "Song is loading, please wait a moment and try again!"));
                return;
            }
        ...
        if (type == "radio_online")
        {
            ...
            // Radio is streamed from URL; offline list stores metadata only.
            app.playlist_offline.Add(this.data_music_cur);
            isSaved = true;
        }

        if (type == "sound_online")
        {
            if (this.data_music_save == null || this.data_music_save.Length == 0)
            {
                app.carrot.Show_msg(app.carrot.L("playlist", "Playlist"), app.carrot.L("save_song_loading", "Song is loading, please wait a moment and try again!"));
    ...
    {
            if (this.data_music_cur["lyrics"] != null)
            {
                ...
                txt_lyrics.text = Regex.Replace(this.data_music_cur["lyrics"].ToString(), "<.*?>", string.Empty);
