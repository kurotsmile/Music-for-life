...
public class App : MonoBehaviour
{
    ...
    private bool is_boot_ready = false;
    private bool pending_start_online = false;
    private bool pending_start_offline = false;
    ...
    void Start()
    {
        ...
        this.is_boot_ready = true;
        if (this.pending_start_offline)
        {
            this.pending_start_offline = false;
            this.start_app_offline();
        }
        else if (this.pending_start_online)
        {
    ...
    {
        if (!this.is_boot_ready)
        {
            Debug.Log("App.start_app_online deferred (boot not ready)");
            this.pending_start_online = true;
            return;
        }

        this.menu_sel = PlayerPrefs.GetInt("menu_sel", 0);
        Debug.Log("App.start_app_online menu_sel=" + this.menu_sel);
        if (PlayerPrefs.GetString("lang_music", "") == "")
            PlayerPrefs.SetString("lang_music", PlayerPrefs.GetString("lang", "en"));
        ...
        {
            int menu_start = this.menu_sel;
            this.carrot.delay_function(0.1f, () => this.open_menu_footer(menu_start));
            this.carrot.delay_function(0.6f, this.Ensure_music_list_loaded_on_start);
            this.carrot.delay_function(2f, this.Ensure_music_list_loaded_on_start);
        }
    ...
    public void start_app_offline()
    {
        if (!this.is_boot_ready)
        {
            this.pending_start_offline = true;
            return;
        }
    ...
    private void After_select_lang(string s_lang)
    {
        ...
        this.menu_sel = 0;
        this.carrot.delay_function(0.1f, () => this.open_menu_footer(0));
        this.carrot.delay_function(0.6f, this.Ensure_music_list_loaded_on_start);
        this.carrot.delay_function(2f, this.Ensure_music_list_loaded_on_start);
    }
    ...
    {
        Debug.Log("App.open_menu_footer index=" + index);
    ...
    private void Ensure_music_list_loaded_on_start()
    {
        if (this.menu_sel != 0) return;
        if (this.canvas_render == null) return;
        if (!this.Should_force_reload_music_view()) return;
        Debug.Log("App.Ensure_music_list_loaded_on_start force reload music");
        this.open_menu_footer(0);
    }

    private bool Should_force_reload_music_view()
    {
        int child_count = this.canvas_render.transform.childCount;
        if (child_count == 0) return true;
        if (child_count == 1 && this.canvas_render.transform.GetChild(0).name == "loading") return true;
        return false;
    }
